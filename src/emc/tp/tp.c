...
STATIC tp_err_t tpCheckAtSpeed(TP_STRUCT * const tp, TC_STRUCT * const tc)
{
    ...
    if (MOTION_ID_VALID(tp->spindle.waiting_for_index)) {
        if (emcmotStatus->spindle_status[tp->spindle.spindle_num].spindle_index_enable) {
            ...
            tp->spindle.revs = 0;
            tp->spindle.offset = 0.0;
            rtapi_print_msg(RTAPI_MSG_INFO, "Index: pending_offset=%.4f spindleRevs=%.4f\n",
...
int tpSetSpindleSync(TP_STRUCT * const tp, int spindle, double sync, int mode, double angular_offset_degrees) {
    rtapi_print_msg(RTAPI_MSG_INFO, "tpSetSpindleSync: sync=%.4f angle_deg=%.2f pending_offset=%.4f\n",
